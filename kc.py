...
def save_paper(conn, rec, file_hash, file_path, orig_filename, client=None):
    cur = conn.execute(
        ...
        "research_design, categories_json, missing_notes, added_at) "
        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        (file_hash, str(file_path), orig_filename, rec["std_name"], rec.get("journal"),
         ...
         json.dumps(rec.get("categories") or []), rec.get("missing_notes"),
...

{
  ...
  "logical_flow": string,            // ONE paragraph on how the ARGUMENT develops:
                                     //   problem -> proposed mechanisms -> connecting assumptions
                                     //   -> resulting hypotheses/insights. Theory over validation;
                                     //   do not dwell on robustness/placebo tests.
  "research_design": string,         // briefly name and explain the PRIMARY design
                                     //   (e.g., difference-in-differences, RD, IV, RCT, event study,
                                     //   structural estimation, descriptive, qualitative, theory model).
                                     //   Not an inventory of datasets; not about robustness tests.
...
def _write_xlsx(papers, datasets):
    ...
    for sheet, ws_ in (("Papers", ws), ("Dataset Universe", ws2)):
...
<body><div class="wrap">
  ...
  <footer>Searchable from anywhere · your PDFs stay on your drive — the file link below each paper opens only on the computer that holds them</footer>
</div>
<script>
...
function paperCard(p){
  ...
  const fileLink = p.file_path? `<div class="path">file: <a href="file://${encodeURI(p.file_path)}">${esc(p.file_path)}</a></div>`:"";
...
function renderDatasets(){
  ...
  const out=datasets.filter(d=>!q||[d.provider,d.product,d.description,d.topic_tags,d.aliases].join(" ").toLowerCase().includes(q));
  document.getElementById("datasets").innerHTML = out.length?out.map(dsCard).join(""):`<div class="empty">No datasets match.</div>`;
  document.getElementById("dcount").textContent = out.length+" / "+datasets.length+" datasets";
}
